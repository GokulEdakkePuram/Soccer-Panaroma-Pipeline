...
CameraTarget PassthroughCamera::update(int64_t /*frame_idx*/, const std::vector<Detection> &detections,
                                       const std::optional<CameraTarget> &prev_target, int frame_width,
                                       int frame_height)
{
    ...
    if (count > 0)
    {
        ...
        {
            target.center_x = prev_target->center_x + kSmoothingAlpha * (measured_x - prev_target->center_x);
            target.center_y = prev_target->center_y + kSmoothingAlpha * (measured_y - prev_target->center_y);
        }
        else
        {
