...
bool StubVideoReader::open(const std::string &path, const Segment &segment)
{
    ...
    std::clog << "StubVideoReader: opened video " << path << ", " << metadata_.width << "x" << metadata_.height << " @ "
              << metadata_.fps << " fps, total frames: " << metadata_.total_frames << '\n';
...
std::optional<Frame> StubVideoReader::read_next()
{
    ...
    frame.timestamp_ms = metadata_.fps > 0.0 ? static_cast<double>(frame.frame_idx) * 1000.0 / metadata_.fps : 0.0;
