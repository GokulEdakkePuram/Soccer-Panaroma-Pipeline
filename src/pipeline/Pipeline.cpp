...
Pipeline::Pipeline(std::unique_ptr<IVideoReader> reader, std::unique_ptr<IDetector> detector,
                   std::unique_ptr<ICamera> camera, std::unique_ptr<IVisualizer> visualizer,
                   std::unique_ptr<IMetrics> metrics, PipelineConfig config)
    : reader_(std::move(reader)), detector_(std::move(detector)), camera_(std::move(camera)),
      visualizer_(std::move(visualizer)), metrics_(std::move(metrics)), config_(std::move(config))
{
}
...
bool Pipeline::run()
{
    ...
    if (metadata_.width > 0 && metadata_.height > 0)
    {
        ...
        warmup.data.assign(static_cast<std::size_t>(warmup.width) * static_cast<std::size_t>(warmup.height) * 3, 0);
        detector_->process(warmup);
    }

    const double frame_budget_ms = metadata_.fps > 0.0 ? 1000.0 / metadata_.fps : 40.0;
    const std::size_t queue_depth = config_.queue_depth > 0 ? static_cast<std::size_t>(config_.queue_depth) : 1;
    ...
    std::thread detector_thread([&] {
        try
        {
            while (auto item = read_queue.pop())
            {
                ...
                if (!detect_queue.push(
                        DetectedItem{std::move(item->frame), std::move(detections), item->reader_ms, detector_ms}))
                {
                    break;
                }
            }
    ...
    try
    {
        while (auto item = detect_queue.pop())
        {
            ...
            CameraTarget target = camera_->update(item->frame.frame_idx, item->detections, prev_target_,
