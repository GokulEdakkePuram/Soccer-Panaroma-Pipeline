...
class YoloV8Detector : public IDetector
{
  public:
    ...
    explicit YoloV8Detector(const std::string &model_path, float conf_threshold = kDefaultConfThreshold,
  ...
  private:
    ...
    std::vector<Detection> decode_output(const cv::Mat &output, const LetterboxParams &letterbox, int frame_w,
                                         int frame_h) const;
